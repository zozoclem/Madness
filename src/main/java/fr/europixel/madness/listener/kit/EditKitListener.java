package fr.europixel.madness.listener.kit;
...
public class EditKitListener implements Listener {
    ...
    public void onClick(InventoryClickEvent event) {
        ...
        if (rawSlot == saveSlot) {
            ...
            if (!isValidHotbar(edited)) {
                player.sendMessage(ConfigUtil.color(plugin.getConfig().getString("messages.editkit.invalid", "&cInvalid hotbar.")));
                player.sendMessage(ConfigUtil.color(plugin.getConfig().getString("messages.editkit.invalid-details", "&7You must keep all default kit items.")));
                return;
            }

            if (plugin.getBlockShopManager() != null) {
                edited = plugin.getBlockShopManager().applySelectedBlock(player, edited);
            }

            plugin.getPlayerStatsManager().setHotbar(player, edited);
            plugin.getPlayerStatsManager().savePlayer(player);

            player.sendMessage(ConfigUtil.color(plugin.getConfig().getString("messages.editkit.saved", "&aYour hotbar has been saved.")));
        ...
        if (rawSlot == cancelSlot) {
            event.setCancelled(true);
            player.sendMessage(ConfigUtil.color(plugin.getConfig().getString("messages.editkit.cancelled", "&cEdit cancelled.")));
            player.closeInventory();
            return;
        }
    ...
    private boolean isValidHotbar(ItemStack[] hotbar) {
        Map<Material, Integer> currentCounts = countNonBlockItems(hotbar);
        Map<Material, Integer> requiredCounts = countNonBlockItems(plugin.getKitManager().createDefaultHotbar());
        ...
        int requiredBlockAmount = plugin.getBlockShopManager() == null ? 0 : plugin.getBlockShopManager().getRequiredBlockAmount();
        int currentBlockAmount = plugin.getBlockShopManager() == null ? 0 : plugin.getBlockShopManager().countOwnedBlockAmount(hotbar);

        return currentBlockAmount >= requiredBlockAmount;
    }

    private Map<Material, Integer> countNonBlockItems(ItemStack[] items) {
        ...
        for (ItemStack item : items) {
            ...
            if (plugin.getBlockShopManager() != null && plugin.getBlockShopManager().isPlaceableBlock(item)) {
                continue;
            }
